...
from dominio_banco.entidades.matricula import Matricula
from dominio_banco.dados.matricula_dao import MatriculaDao
...
def executar_exemplo():
    print("="*50)
    print("EXEMPLO DE UTILIZAÇÃO GERADO AUTOMATICAMENTE - MATRICULA")
    print("="*50)

    # Conexão de teste apontando para o banco em uso
    conexao = sqlite3.connect("meu_banco.db")
    dao = MatriculaDao(conexao)

    print("\n1. GERANDO DADOS ALEATÓRIOS (Conforme exigência do Trabalho 2)...")
    novo_obj = Matricula(
        id_matricula=None,
        id_aluno=random.randint(1, 1000),
        id_curso=random.randint(1, 1000),
        data_matricula=gerar_string_aleatoria(15),
        status=gerar_string_aleatoria(15)
    )
